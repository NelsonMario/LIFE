import symptomCheckerClient
import random
import config
import sys
import json

class SymptomCheckerClientDemo:
    'Demo class to simulate how to use symptomCheckerClient'
    ...
    def __init__(self):
        ...
        self._diagnosisClient = symptomCheckerClient.SymptomCheckerClient(username, password, authUrl, language, healthUrl)
    ...
    def simulate(self):
        ...
        selectedSymptoms = self._loadSublocationSymptoms(selectedLocationID)
    ...
    def _loadBodyLocations(self):
        ...
        pick = int(input("Input body location"))
        clientBodyLocation = bodyLocations[pick]
        self._writeHeaderMessage("Sublocations for randomly selected location {0}".format(bodyLocation["Name"]))
        return clientBodyLocation["ID"]
    ...
    def _loadBodySublocations(self, locId):
        ...
        pick = int(input("Input body sub location"))
        clientBodySublocation = bodySublocations[pick]
        self._writeHeaderMessage("Sublocations for randomly selected location {0}".format(clientBodySublocation["Name"]))
        return clientBodySublocation["ID"]


    def _loadSublocationSymptoms(self, subLocId):
        symptoms = self._diagnosisClient.loadSublocationSymptoms(subLocId, symptomCheckerClient.SelectorStatus.Man)
        ...
        pick = int(input("Input symptoms"))
        clientSymptom = symptoms[pick]

        self._writeHeaderMessage("Randomly selected symptom: {0}".format(clientSymptom["Name"]))

        self._loadRedFlag(clientSymptom)

        selectedSymptoms = [clientSymptom]
    ...
    def _loadDiagnosis(self, selectedSymptoms):
        ...
        diagnosis = self._diagnosisClient.loadDiagnosis(selectedSymptomsIds, symptomCheckerClient.Gender.Male, 1988)
        ...
        for d in diagnosis:
            ...
            dictDiagnosis = {
                "name"    : d["Issue"]["Name"],
                "acc"     : d["Issue"]["Accuracy"], 
                "Icd"     : d["Issue"]["Icd"], 
                "IcdName" : d["Issue"]["IcdName"], 
                "specialization": ",".join(x for x in specialisations)
            }

            self.diagnosisResult.append(dictDiagnosis)
    ...
    def _loadSpecialisations(self, selectedSymptoms):
        ...
        specialisations = self._diagnosisClient.loadSpecialisations(selectedSymptomsIds, symptomCheckerClient.Gender.Male, 1988)
    ...
    def _loadProposedSymptoms(self, selectedSymptoms):
        ...
        proposedSymptoms = self._diagnosisClient.loadProposedSymptoms(selectedSymptomsIds, symptomCheckerClient.Gender.Male, 1988)
...
diagnosisRes = []
diagnosisClientDemo = SymptomCheckerClientDemo()
